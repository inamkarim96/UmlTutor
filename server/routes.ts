...
async function runConsistencyCheck(diagram: any) {
  ...
  if (diagram.type === "usecase") {
    ...
    content.associations?.forEach((a: any) => {
      connected.add(a.source);
      connected.add(a.target);
    });
    content.actors?.forEach((a: any) => {
    ...
    content.useCases?.forEach((u: any) => {
...
function calculateConsistencyScore(issues: any[]) {
  return Math.max(
    0,
    100 - issues.reduce((acc, i) => acc + (i.severity === "error" ? 20 : 10), 0)
  );
}
